...
object RoleStore {
    ...
    fun setPrimaryRoles(
        ...
        record.bodyModel = normalizeBodyModel(femaleGender.bodyModel)
        record.fgBustSize = normalizeFemaleGenderBustSize(femaleGender.bustSize)
        record.fgPhysics = femaleGender.physics
        record.fgShowInArmor = femaleGender.showInArmor
        record.fgBounce = normalizeFemaleGenderBounce(femaleGender.bounce)
        record.fgFloppy = normalizeFemaleGenderFloppy(femaleGender.floppy)
        record.activeJobIds.clear()
        record.activeJobIds.add(jobId)
        record.activeClassIds.clear()
        record.activeClassIds.add(classId)
    ...
    fun resetOnboarding(player: ServerPlayer) {
        ...
        record.bodyModel = DEFAULT_BODY_MODEL
        record.fgBustSize = DEFAULT_FG_BUST_SIZE
        record.fgPhysics = DEFAULT_FG_PHYSICS
        record.fgShowInArmor = DEFAULT_FG_SHOW_IN_ARMOR
        record.fgBounce = DEFAULT_FG_BOUNCE
        record.fgFloppy = DEFAULT_FG_FLOPPY
        save()
        ClassSkillTrees.reconcile(player)
    }
...
class PlayerRoleRecord(
    ...
    var bodyModel: String = DEFAULT_BODY_MODEL,
    var fgBustSize: Double = DEFAULT_FG_BUST_SIZE,
    var fgPhysics: Boolean = DEFAULT_FG_PHYSICS,
    var fgShowInArmor: Boolean = DEFAULT_FG_SHOW_IN_ARMOR,
    var fgBounce: Double = DEFAULT_FG_BOUNCE,
    var fgFloppy: Double = DEFAULT_FG_FLOPPY,
...
fun normalizeBodyModel(value: String): String = if (value.equals(BODY_MODEL_GIRL, ignoreCase = true)) BODY_MODEL_GIRL else BODY_MODEL_BOY
