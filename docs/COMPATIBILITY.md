...
- NPC TOML can set `body_model`, `fg_bust_size`, `fg_bounce`, and `fg_floppy`. CKDM checks that `wildfire_gender` is installed, then renders a CKDM-owned Female Gender-style wedge bust layer on normal/playerlike NPC `PlayerModel` renderers using the NPC skin texture. Gecko custom-animation NPCs are excluded because they do not use a vanilla `HumanoidModel` parent. Female Gender's own `GenderLayer` is not used for NPCs because it only resolves breast textures for `AbstractClientPlayer`.
