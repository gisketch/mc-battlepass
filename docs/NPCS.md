...
Shared generic messages live in `friendship_messages.json` and affect every NPC. NPC JSON files can add their own message lines under the same keys; those NPC-specific lines join the shared random pool with higher weight.

Global greeting behavior lives in `settings.json`:
...
Example shared `friendship_messages.json` addition:
...
Example NPC-specific addition inside an NPC JSON file:
