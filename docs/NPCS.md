...
- `/npc fight <class_id>`: OP-only temporary boss duel that spawns a transient test NPC using the requested boss moveset id, such as `warrior`, `rogue`, `archer`, `wizard`, `arcane_wizard`, `priest`, `bard`, or `witcher`. The debug NPC keeps its generated boss id for cleanup but borrows the skin/body type from the first configured NPC with that class.
