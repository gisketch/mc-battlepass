...
Paladin/Tarnished uses the `paladin` boss template with `minecraft:mace` and `minecraft:shield`. It is final-boss Elden-style pressure: close guard counter, shield bash, mace heavy, golden slam, holy shock, judgement, absorption-only golden barrier/battle banner, medium and back rolls, phase-2 holy beam, phase-2 Erdtree burst hazard, visible holy/shield particles, and no teleport or defensive waiting loop.
