...
Explorer-map stores are dynamic store offers, not a separate UI. A store offer with `offer_type = "explorer_map"` uses the existing daily/weekly store pools but resolves and caches its stock per player during store refresh, not while opening the store UI. Put common maps under `daily_items`, rare maps under `weekly_items`, or bundled rotations under `daily_sets`/`weekly_sets`; each offer declares `target_type = "biome"` or `"structure"` and a `targets` list of biome/structure ids or tags. CKDM stores a per-player explorer journal at `<world>/data/gisketchs_chowkingdom_mod/explorer_maps/state.json`, records visited biomes, and avoids reselling visited biome maps or already sold target maps. Default range is `512..12000` blocks; targets outside the configured range are not shown as stock.
