...
Buy opens the store configured on the NPC definition. Gift is disabled unless the player is holding an item, shows a hover hint, consumes one held item on success, and plays a reaction dialog. Bye closes the screen. Talk is reserved for a future conversation input flow.
...
Gift limits are per NPC/player and reset by in-game schedule period. Finn defaults to one gift per in-game day, resetting at 05:00. Gift reactions are only flavor dialog right now; no friendship mechanic is stored.
