...
Buy opens the store template configured on the NPC job definition. NPC stores share the same store TOML pool by store id, but each NPC uses its own stock key, so two NPCs can roll different daily/weekly items and deplete stock independently. WORK grants a job application if the NPC has no workplace, or opens workplace management with MOVE and FIRE if one is assigned. MOVE gives a fresh job application for reassignment. FIRE clears the assigned workplace and leaves the NPC unemployed until WORK is used again. If configured `work_blocks` are missing around the workplace, assignment fails or the shop stays closed, and the NPC opens a close-only missing-block dialog. Training requires the NPC to have an assigned workplace, then checks the same class license rules as class commands; success unlocks and activates the NPC's configured class and grants class starting items. If the only blocker is a full starter or upgrade license pool, the failed Training reply can show CHANGE with a chowcoin icon. CHANGE opens a selection list of owned same-type classes to replace; starter changes cost 50,000 chowcoins and upgrade changes cost 100,000 chowcoins. Replacing a starter class warns when upgrade classes would lose all starter prerequisites, and confirming removes those invalid upgrades too. Other failures stay close-only with the missing workplace, license, or prerequisite conditions. After a successful NPC store purchase, the owning NPC opens a close-only follow-up dialog chosen from `shop_messages` by friendship category and quantity bucket. Gift is disabled unless the player is holding an item, shows a hover hint, consumes one held item on success, and plays a reaction dialog with a single OKAY close button. Locked relic/player-locked items are rejected by the existing relic transfer guard. Bye closes the normal screen.
...
Workplace dialogs can use LLM text when `llm_message_usage.work_application`, `work_manage`, `work_move`, `work_fire`, `assigned_workplace`, or `work_missing_blocks` are enabled. Prompt text for those surfaces lives under `[work]` in NPC `settings.toml`. Missing work-block prompts receive `{missing}`, `{requirements}`, `{workplace}`, and `{action}`. Class training dialogs can use LLM text when `llm_message_usage.class_training` is enabled. Training prompts live under `[training]`; failed prompts receive `{player}`, `{npc}`, `{class}`, `{conditions}`, `{overall_level}`, `{cost}`, `{classification}`, `{change_options}`, and `{lost_classes}`. Missing-workplace training uses `workplace_required_llm_prompt`. Paid change dialogs use `change_offer_message`, `change_select_message`, `change_success_message`, `change_failed_funds_message`, `change_invalid_message`, and `change_lost_upgrades_warning`; `change_offer_llm_prompt` lets NPCs pitch the target class with playful rivalry toward replaceable classes.
...
- `class`: Optional class id for Training dialog classification, for example `class = "rogue"`. The NPC does not use class mechanics yet; this only decides what class the NPC can teach.
...
- `boss`: Optional boss duel knobs: `enabled`, `health`, `damage`, `template`, and nested `boss.balloons`. Defaults allow OP-only `/npc fight` testing on any NPC with the current `sword_user` template. Boss balloons are data-driven short world-space lines for `chase`, `attack`, `recovery`, `taunt`/guard bait, `guard_react`, `parry`, `hit_player`, `took_damage`, `victory`, and `defeat`. Supported placeholders: `{player}`, `{npc}`, `{boss}`, `{phase}`, `{health}`, and `{max_health}`.
...
OPs can run `/npc fight` while looking at any NPC to start a temporary non-lethal boss duel. The V1 boss loop uses the same custom animation path with `running_sword`, `attack`, `hurt`, `guard`, and `parry`: it chases, commits to an attack, enters a short slower `running_sword` recovery window, then always enters guard bait before chasing again. Recovery allows exactly 1 safe punish hit; any extra swing after that hit immediately converts into guard/parry. The recovery and guard-bait side-step slow both movement and Gecko animation playback. Guard bait keeps the slower `running_sword` side-step until the player hits or the 3-6 second bait window expires; that hit is blocked before vanilla NPC hurt visuals/knockback, `guard` flashes, parry particles and sound play, `parry` plays, and the player is knocked back. The bossbar title shows the live `NPC mode` such as `chase`, `attacking`, `recovery`, `guard`, or `parry`; the duelist does not get repeated actionbar status spam. Bossfight chat balloons are selected from each NPC's `[boss.balloons]` pools on phase changes, guard taunts, landed boss hits, accepted recovery damage, guard reactions, parries, NPC victory, and player defeat. Each bark rolls a 30% chance so the NPC does not talk on every trigger. Player damage reduces virtual boss health only during recovery instead of killing the NPC. If a boss hit would down the player, the hit is cancelled before the revive system, the duel ends as an NPC victory, the player is restored to full health, and the NPC opens a close-only victory dialog using LLM when enabled. Boss duels are isolated 1v1s: third-party entity damage into either participant is blocked, and the duelist and boss cannot damage outside targets while the fight is active. Right-click NPC interaction is blocked during the fight, so dialogue, gifts, quests, and join-talk prompts cannot interrupt combat. The duelist sees no interaction warning; spectators get a snackbar if they try to interact. On boss defeat, the bossbar clears, the NPC restores its previous custom animation and held items, and the NPC opens a close-only defeat dialog using LLM when enabled.
