...
Items whose recipe ids are disabled show a gray `Recipe disabled` tooltip row. For modded conversion recipes, the tooltip mapper also recognizes common recipe-id shapes like `item_from_other_item` and `folder/item`.
