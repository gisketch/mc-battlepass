...
def topic_counts(entries):
    counts = collections.Counter()
    for entry in entries:
        counts[str(entry.get("topic") or "")] += 1
    return counts
...
def main():
    ...
    if args.summary or not args.validate:
        ...
        for topic, count in topic_counts(exchanges + trainer).most_common()[-20:]:
            print(f"{topic}: {count}")
    ...
    if args.topic:
        matching = [entry for entry in exchanges + trainer if entry.get("topic") == args.topic]
        print(f"topic={args.topic} count={len(matching)}")
